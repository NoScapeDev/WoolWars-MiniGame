...
public class EskimoAbility extends Ability {
    ...
    public EskimoAbility() {
        super("Eskimo", new ItemBuilder(Material.PACKED_ICE).name("&bEskimo").lore(Arrays.asList(
    ...
    public AbilityCallable getAbilityCallable() {
        return player -> {
            ...
            if (player.getLocation().subtract(0, 1, 0).getBlock().getType() == Material.AIR) {
                player.sendMessage(Utils.format("&cYou have to be standing on the floor to use the ability"));
