...
public class ShurikenAbility extends Ability {
    ...
    public ShurikenAbility() {
        super("Shuriken", new ItemBuilder(Material.NETHER_STAR).name(Utils.format("&bShuriken")).lore(Arrays.asList(
    ...
    public void throwShuriken(Player player) {
        ...
        this.shuriken.put(player, Pair.of(item, this.main.getServer().getScheduler().runTaskLater(this.main, () -> removeShuriken(player), 60L)));
    }
    ...
    public boolean hitPlayer(Player player, Player target) {
        target.getWorld().playSound(player.getLocation(), Sound.BLOCK_ANVIL_FALL, 20.0F, 10.0F);
